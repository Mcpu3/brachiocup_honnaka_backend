...
def read_item(database: Session, item_uuid: str) -> Optional[models.Item]:
    return database.query(models.Item).filter(models.Item.uuid == item_uuid).first()

def read_item_by_barcode(database: Session, group_uuid: str, barcode: str) -> Optional[models.Item]:
    return database.query(models.Item).filter(and_(models.Item.group_uuid == group_uuid, models.Item.barcode == barcode)).first()
